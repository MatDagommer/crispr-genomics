...
# Added by Matthieu
#----------------------------------
parser.add_argument("--reg", default = False, action = 'store_true', help= "Perform regression instead of classification.")
#----------------------------------
...
def main():
    ...
    for i in range(0, args.repeat):
        print('Iteration', i)
        model = DNN(pdata, f_tp, f_fp, f_th, expName, i, result_dir=args.result_dir, reg = args.reg)
        evaluationDict = model.getEvaluationStat()

        print(evaluationDict)

        saveEvaluation(evaluationDict, fw, i + 1, args.reg)
...
def saveEvaluation(evaluationDict, fw, iteration, reg):
    if not reg:
        ...
        fw.write(str(iteration) + "\t" + str(evaluationDict['mse']) + '\n')
