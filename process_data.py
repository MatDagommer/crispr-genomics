...
def splitDataset(essFeatTable, nessFeatTable, trainingProp, fold=4):
    ...
    # Added by Matthieu
    #--------------------------------------------------------------------------------------------------------
    ess_num = math.ceil(essFeatTable.shape[0] * fold)
    essTotal = essFeatTable.shape[0]
    if ess_num <= essTotal:
        essTable = essFeatTable[np.random.choice(essTotal, ess_num, replace=False), :]
    else:
        essTable = essFeatTable[np.random.choice(essTotal, ess_num, replace=True), :]
    
    # calculating training, validation, testing data portion
    validationProp, testingProp = float(1 - trainingProp) / 2, float(1 - trainingProp) / 2
    
    # shuffling the data to mix the data before splitting the dataset into training, validation and testing data
    nessTable = nessFeatTable
    np.random.shuffle(nessFeatTable)
    np.random.shuffle(essTable)
    #--------------------------------------------------------------------------------------------------------
    """
